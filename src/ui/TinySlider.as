package ui
{
	...
	public class TinySlider extends Sprite 
	{
		...
		public function TinySlider(onChange:Function, label:String = "", plusMinus:Boolean = false, width:int = 100, height:int = 9)
		{
			...
			if (plusMinus)
			{
				_border.graphics.moveTo(_width / 2, 0);
				_border.graphics.lineTo(_width / 2, -1);
				_border.graphics.moveTo(_width / 2, _height);
				_border.graphics.lineTo(_width / 2, _height + 1);
		...
		{
			_newValue = _plusMinus ? (mouseX / _width) * 2 - 1 : mouseX / _width;
			...
			_bar.scaleX = _plusMinus ? _newValue * 0.5 : _newValue;
			_bar.x = _plusMinus ? _width / 2 : 0;
